...
else
{
  ...
  <?php if($row['Status']=="Sorted")
  {
    ...
    $tomorrow = date($today, strtotime('+1 day'));
    $link_expiration_date = date('Y-m-d H:i:s', strtotime($examdate . ' +1 day'));
    
    if($examdate <= $today && $link_expiration_date > $today ){
      echo "<td>";
      $link = $row["jobexam"];
      echo "<a href='".$link."' target=\"_blank\">".$row['jobexam'] ."</a>";
      echo "<br><br> Exam Link Will Expire: $link_expiration_date";
      echo "</td>";
    }
    elseif($link_expiration_date < $today){
      echo "<td>";
      echo "<h5 class='text-danger'>Exam Link Expired</h5>";
      echo "</td>";
    }
    else{
      echo "<td>";
      echo "<strong class='text-info'>Exam Link Will appear here on Exam Date!!</strong>";
      echo "</td>";
    } 
    echo "</tr>";
  }
