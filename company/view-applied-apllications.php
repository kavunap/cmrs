...
<?php  if($row['Status']!=''){
$ret=mysqli_query($con,"select  tblmessage.ID,tblmessage.Message,tblmessage.Status as comstatus,tblmessage.ResponseDate from tblapplyjob  left join tblmessage on tblmessage.AppID=tblapplyjob.ID where tblapplyjob.ID='$vid'");
$cnt=1;
...
<tr>
 ...
 <td><?php  echo $row1['Message'];?></td> 
  <td><?php  echo $row1['comstatus'];?></td> 
   ...
   <td><a href="edit-message.php?editid=<?php echo $row1['ID'];?>">Edit Details</a></td>
</tr>
<?php $cnt=$cnt+1;} ?>
</table>
<?php  }  
